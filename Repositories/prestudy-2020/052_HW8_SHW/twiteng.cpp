...
bool TwitEng::parse(char* filename)
{
	...
	for(int i = 0; i <= user_count; i++)
	{
		...
		ss >> password;
		master_users[new_user]->setPassword(password);
...
User* TwitEng::find(std::string username)
{
	if(master_users.find(username) != master_users.end())
	{
		return (master_users.find(username))->second;
	}
	return NULL;
}
...
{
	std::map<std::string, User*>::iterator follower_it;
	follower_it = master_users.find(name_of_follower);

	std::map<std::string, User*>::iterator following_it;
	following_it = master_users.find(name_to_follow);

	if(follower_it != master_users.end() && following_it != master_users.end())
	{
		follower_it->second->addFollowing(following_it->second);
		following_it->second->addFollower(follower_it->second);
	}

	if(master_users.find(name_of_follower) == master_users.end())
	{
    	std::cout << name_of_follower << " is not a current user" << std::endl;
    	return;
    }
    else if(master_users.find(name_to_follow) == master_users.end())
	{
		std::cout << name_to_follow << " is not a current user" << std::endl;
    	return;
    }
    else if(master_users.find(name_to_follow) == master_users.end() 
    	&& master_users.find(name_of_follower) == master_users.end())
    {
...
void TwitEng::create_save_db(std::string filename)
{
	...
	for(std::map<std::string, User*>::iterator it2 = master_users.begin();
		it2 != master_users.end();
		it2++)
	{
...
void TwitEng::login(User* user)
{
	logged_in = user;
}

User* TwitEng::currUser()
{
	return logged_in;
}